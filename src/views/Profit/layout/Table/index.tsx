 ...
 * @LastEditTime: 2022-05-05 16:35:58
 */
...
import { NAlert, NDataTable, NIcon, NSpace, useMessage } from "naive-ui";
...
export default defineComponent({
    ...
    setup() {
        ...
        const links = ref<{ address?: string, block?: string, tx?: string } | undefined>(null);
        const tableData = useService<Page<Record<string, any>[]>>(getPaymentList, { immediate: true, params: () => paymentSearch.getData(), defaultValue: new Page<Record<string, any>[]>() }, (data, res) => {
            paymentSearch.itemCount = data.pages
            links.value = res.ext
            message.value = res.ext.remindMsg
        });
    ...
    render() {
        return (
            <div>
                {
                    this.message && <NAlert type="warning" showIcon={false} closable>
                        <div class="flex items-center gap-3">
                            <NIcon class="text-current">
                ...
                <NDataTable class="table-base" pagination={this.paymentSearch} scrollX={910} loading={this.tableData.loading} data={this.tableData.data.records} columns={this.columns} size="large" />
            </div>
        )
    }
})